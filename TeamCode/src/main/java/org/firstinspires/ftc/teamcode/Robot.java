...
public class Robot {
    ...
    public void goToPoint(Pose2d pose, LinearOpMode opMode) {
        ...
        while((Math.abs(pose.x-ROBOT_POSITION.x) > 2 || Math.abs(pose.y-ROBOT_POSITION.y) > 2 || Math.abs(pose.heading - ROBOT_POSITION.heading) > Math.toRadians(5)) && opMode.opModeIsActive()) {
            update();
        }
    }
